...
def emotion_detector(text_to_analyse):  # Define a function named sentiment_analyzer that takes a string input (text_to_analyse)
    ...
    response = requests.post(url, json = myobj, headers=header)  # Send a POST request to the API with the text and headerss
    return response.text  # Return the response text from the API
dict_response = json.loads(emotion_detector('i am so happy doing this'))
